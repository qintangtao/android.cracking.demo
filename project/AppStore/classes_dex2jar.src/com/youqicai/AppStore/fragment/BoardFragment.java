  ...
  implements TaskEntity.OnResultListener, PullToRefreshBase.OnRefreshListener2, InstallReceiver.AppInstallListener, RefreshListener
{
  ...
  private static final String RANK_TYPE_QUICK = "quick";
  ...
  public Handler mHandler = new Handler()
  {
    public void handleMessage(Message paramAnonymousMessage)
    {
      ...
      {
        return;
        BoardFragment.this.appInfoAdapter.setAll(BoardFragment.this.mDataList);
        BoardFragment.this.appInfoAdapter.notifyDataSetChanged();
        BoardFragment.this.setViewShowStatus(2);
        return;
        localArrayList = (ArrayList)paramAnonymousMessage.obj;
      }
      while (localArrayList == null);
      BoardFragment.this.mDataList.addAll(localArrayList);
      for (int i = 0; i < BoardFragment.this.mDataList.size(); i++)
        ((AppInfoEntity)BoardFragment.this.mDataList.get(i)).setRank(i + 1);
      BoardFragment.this.appInfoAdapter.setAll(BoardFragment.this.mDataList);
      BoardFragment.this.appInfoAdapter.notifyDataSetChanged();
      BoardFragment.this.setViewShowStatus(2);
    }
  ...
  {
    BoardFragment localBoardFragment = new BoardFragment();
    Bundle localBundle = new Bundle();
    localBundle.putInt("position", paramInt);
    localBoardFragment.setArguments(localBundle);
    return localBoardFragment;
  }
  ...
  public void onActivityCreated(Bundle paramBundle)
  {
    ...
    HttpUtil.AddTaskToQueueHead(BuildUrl.buildBillBoard("quick", this.pageNo), 0, new UpdateParser(), this);
    setViewShowStatus(0);
  ...
  {
    HttpUtil.AddTaskToQueueHead(BuildUrl.buildBillBoard("quick", this.pageNo), 1, new UpdateParser(), this);
  }
  ...
  public class CheckDownloadStatusThread extends Thread
  {
    ...
    public void run()
    {
      if ((BoardFragment.this.mDataList != null) && (BoardFragment.this.mDataList.size() > 0))
      {
        ArrayList localArrayList = BoardFragment.this.mDataList;
        ApkManager.getInstance().checkDownloadStatus(localArrayList);
        Message localMessage = Message.obtain();
        localMessage.what = 0;
        BoardFragment.this.mHandler.sendMessage(localMessage);
      }
      super.run();
    }
  }
 ...
 * Qualified Name:     com.youqicai.AppStore.fragment.BoardFragment
