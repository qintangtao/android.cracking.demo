...
public class SoftwareFragment extends BaseFragment
  implements TaskEntity.OnResultListener, PullToRefreshBase.OnRefreshListener2<ListView>, InstallReceiver.AppInstallListener, RefreshListener
{
  ...
  public Handler mHandler = new Handler()
  {
    public void handleMessage(Message paramAnonymousMessage)
    {
      ...
      {
        return;
        SoftwareFragment.this.cardAdapter.setAll(SoftwareFragment.this.mDataList);
        SoftwareFragment.this.cardAdapter.notifyDataSetChanged();
        SoftwareFragment.this.setViewShowStatus(2);
        return;
        localList = (List)paramAnonymousMessage.obj;
      }
      while (localList == null);
      SoftwareFragment.this.mDataList.addAll(localList);
      SoftwareFragment.this.cardAdapter.setAll(SoftwareFragment.this.mDataList);
      SoftwareFragment.this.cardAdapter.notifyDataSetChanged();
      SoftwareFragment.this.setViewShowStatus(2);
    }
  ...
  public void onActivityCreated(Bundle paramBundle)
  {
    ...
    this.pageNo = 1;
    HttpUtil.AddTaskToQueueHead(BuildUrl.buildSoftList(this.pageNo), 0, new BaseCardParser(), this);
    setViewShowStatus(0);
  ...
  {
    HttpUtil.AddTaskToQueueHead(BuildUrl.buildSoftList(this.pageNo), 1, new BaseCardParser(), this);
  }
  ...
  public class CheckDownloadStatusThread extends Thread
  {
    ...
    public void run()
    {
      if ((SoftwareFragment.this.mDataList != null) && (SoftwareFragment.this.mDataList.size() > 0))
      {
        ArrayList localArrayList = new ArrayList();
        int i = 0;
        if (i < SoftwareFragment.this.mDataList.size())
        {
          AppInfoEntity localAppInfoEntity = ((CardEntity)SoftwareFragment.this.mDataList.get(i)).getSingleAppItem();
          ...
          while (true)
          {
            i++;
            break;
            List localList = ((CardEntity)SoftwareFragment.this.mDataList.get(i)).getAppItemList();
        ...
        localMessage.what = 0;
        SoftwareFragment.this.mHandler.sendMessage(localMessage);
      }
      super.run();
    }
  }
 ...
 * Qualified Name:     com.youqicai.AppStore.fragment.SoftwareFragment
